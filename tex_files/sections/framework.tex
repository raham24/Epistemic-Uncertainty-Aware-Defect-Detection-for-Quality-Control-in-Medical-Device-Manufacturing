...
where \(p_{i,C+1}\) is the reservation variable for the \(i\)-th sample, subject to the constraint \(\sum_{j=1}^{C} p_{i,j}+p_{i,C+1}=1\) for \(p_j\ge 0 \forall j\in\{1,\ldots,C+1\}\). The parameter \(t>0\) is a hyperparameter that controls the trade-off between making a prediction and abstaining. A higher value of \(t\) encourages the model to make predictions, while a lower value encourages abstention.
