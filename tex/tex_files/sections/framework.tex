...
Let \(\calX\subseteq\R^d\) be the (possibly continuous) feature space and let  \(\calY=\{1,\dots,C\}\) denote the concept of the label space related to some task of interest. Moreover, let \(\pXY\) be the underlying (unknown) \gls*{pdf} over \(\calX\times\calY\).
...
\begin{align}
    ...
    \sum_{j=1}^{C} p_{j}\log\left(\phat_{j}\right).
\end{align}
...
\begin{align}
    ...
    \sum_{j=1}^{C} p_{j}\log\left(t\cdot\phat_{j}+\phat_{C+1}\right),
\end{align}

where \(\phat_{i,C+1}\) is the reservation variable for the \(i\)-th sample, subject to the constraint \(\sum_{j=1}^{C} p_{i,j}+p_{i,C+1}=1\) for \(p_j\ge 0 \forall j\in\{1,\ldots,C+1\}\). The parameter \(t>0\) is a hyperparameter that controls the trade-off between making a prediction and abstaining. A higher value of \(t\) encourages the model to make predictions, while a lower value encourages abstention.
